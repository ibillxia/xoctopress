...
<center>{% img /images/2019/IMAG2019100801.png %}</center><br>
...
- 列出数据库中表列表 show tables;
- 创建数据库 create database test;
- 删除一个表 drop table if exists test.t1;
- 创建第一个表 create /*temporary*/ table /*if not exists*/ test.m1 ( id UInt16,name String) ENGINE = Memory;
- 插入测试数据 insert into test.m1 (id, name) values (1, 'abc'), (2, 'bbbb');
- 查询 select * from test.m1;
...
select * from test.m2;select a, b from test.m2;
```
...
remote_group 是/etc/clickhouse-server/config.xml中remote_servers参数
...
```
create table test.tinylog_d (id UInt16, name String) ENGINE=Distributed(log, test,tinylog_d1 , id);-- 插入数据到逻辑表，观察数据分发情况
...
select * from test.replacingmergetree;-- 如果记录未执行merge，可以手动触发一下 merge 行为
...
insert into test.summingmergetree (sdt, name, a, b) values ('2018-06-12', 'c', 1, 35);-- 手动触发一下 merge 行为
...
select sdt,dim2, dim3, uniqState(dim1) as uv from test.aggregatingmergetreegroup by sdt,dim2, dim3;
