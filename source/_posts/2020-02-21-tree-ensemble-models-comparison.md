---   
layout: post   
title: "集成树模型详解：RandomForest、GBDT、XGBoost与LightGBM"   
date: 2020-02-21 20:00   
comments: true   
categories: DataScience   
tags: 机器学习 随机森林 GBDT XGBoost LightGBM   
---   
...
<center>{% img /images/2020/IMAG2020022101.png %}</center><br>
...
<center>{% img /images/2020/IMAG2020022102.jpeg %}</center><br>

## 1.2 Boosting

Boosting 训练过程为阶梯状，基模型的训练是有顺序的，每个基模型都会在前一个基模型学习的基础上进行学习，最终综合所有基模型的预测值产生最终的预测结果，用的比较多的综合方式为加权法。

<center>{% img /images/2020/IMAG2020022103.jpeg %}</center><br>

## 1.3 Stacking

Stacking 是先用全部数据训练好基模型，然后每个基模型都对每个训练样本进行的预测，其预测值将作为训练样本的特征值，最终会得到新的训练样本，然后基于新的训练样本进行训练得到模型，然后得到最终预测结果。

<center>{% img /images/2020/IMAG2020022104.jpeg %}</center><br>

那么，为什么集成学习会好于单个学习器呢？原因可能有三：

1. 训练样本可能无法选择出最好的单个学习器，由于没法选择出最好的学习器，所以干脆结合起来一起用；

1. 假设能找到最好的学习器，但由于算法运算的限制无法找到最优解，只能找到次优解，采用集成学习可以弥补算法的不足；

1. 可能算法无法得到最优解，而集成学习能够得到近似解。比如说最优解是一条对角线，而单个决策树得到的结果只能是平行于坐标轴的，但是集成学习可以去拟合这条对角线。
...
<center>{% img /images/2020/IMAG2020022106.jpeg %}</center><br>
...
<center>{% img /images/2020/IMAG2020022107.png %}</center><br>
...
<center>{% img /images/2020/IMAG2020022108.png %}</center><br>
...
XGBoost的原理详细，推荐两篇大神博客， [xgboost原理](https://www.cnblogs.com/yuluoxingkong/p/8624787.html) ，[xgboost 算法原理](https://www.cnblogs.com/pinard/p/10979808.html)。
...
1. 列抽样。XGBoost借鉴了随机森林的做法，支持列抽样，不仅防止过 拟合，还能减少计算；

1. 对缺失值的处理。对于特征的值有缺失的样本，XGBoost还可以自动学习出它的分裂方向；
...
[【1】集成学习](https://www.runoob.com/ml/ml-ensemble-learning.html)

[【2】3种常见的集成学习决策树算法及原理](https://cloud.tencent.com/developer/article/2318546)

[【3】【机器学习】决策树（中）——Random Forest、Adaboost、GBDT （非常详细）](https://zhuanlan.zhihu.com/p/86263786)
