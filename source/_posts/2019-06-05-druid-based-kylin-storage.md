---   
layout: post   
title: "【美团】基于Druid的Kylin存储引擎实践"   
date: 2019-06-05 20:54   
comments: true   
categories: BigData   
tags: Kylin Druid OLAP 大数据   
---   

作者: 康凯森

8月11日，由 Kyligence 主办、美团点评协办的 Apache Kylin Meetup@北京，在美团公司总部圆满落幕。本文整理自当天美团大数据工程师、Apache Kylin Committer康凯森的演讲实录，全文共6600字，阅读时间大约15分钟。或点击[视频](https://v.qq.com/x/page/f0763nt7h4o.html)，观看Meetup现场实录。
...
<center>{% img /images/2019/IMAG2019060501.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060502.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060503.png %}</center><br>
...
1. 因为只需要读取必需访问的列，所以列存有高效的IO

1. 因为每列数据的类型一致，格式一致，所以列存可以进行高效的编码和压缩

1. 列存更容易实现向量化执行，而向量化执行相比传统的火山模型，函数调用次数更少，对CPU Cache和SIMD更加友好。 总的来说，列存相比HBase的KV模型更适合Kylin的查询场景。

所以，要解决Kylin On HBase Scan和Filter效率低下的问题, 我们就需要为Kylin增加一个列存，有高效索引的存储引擎。

# 二、Kylin 新存储引擎探索之路

<center>{% img /images/2019/IMAG2019060504.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060505.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060506.png %}</center><br>

我们在调研存储格式时主要考虑Scan和Filter性能，导入性能，压缩率，集成难度这4点因素，其中重点关注Scan和Filter性能。

## Kylin On Parquet POC

<center>{% img /images/2019/IMAG2019060507.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060508.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060509.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060510.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060511.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060512.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060513.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060514.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060515.png %}</center><br>

下面我们来详细看下KOD数据导入和数据查询的过程，我们首先来看下数据导入过程：

1. 和HBase一样，首先需要计算出Cuboid

1. 生成Cuboid后，Kylin On HBase中是将Cuboid转为HBase的HFile，KOD中则是将Cuboid转为Druid Segment文件。

1. 向Druid的MetaData Store插入Segment元数据。下面的工作是Druid内部的流程:

1. Coordinator定期从MetaData Store Pull Segment元数据，发现有新Segment生成时，就会通知Historical节点Load Segment

1. Historical节点收到Load Segment通知后，就会去HDFS上下载Segment文件，然后MMap到内存，至此整个数据导入过程完成。

<center>{% img /images/2019/IMAG2019060516.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060517.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060518.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060519.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060520.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060521.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060522.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019060523.png %}</center><br>
...
1. Kylin的预计算比Druid更加强大，Druid只能计算Base Cuboid

1. Kylin支持基于预计算的精确计算，精确去重是我司强需求

1. Kylin的SQL支持更加完善

1. Kylin的离线导入更加完善

1. Kylin对星型模型的查询支持更加友好
...
1. Kudu 并没有倒排索引或者二级索引

1. Kudu 是C++实现的，我们团队的技术栈主要是Java，我们的存储团队也没有引进Kudu
