---   
layout: post   
title: "Flink原理与实现：数据流上的类型和操作"   
date: 2020-03-01 17:50   
comments: true   
categories: BigData   
tags: Flink 流计算 DataStream   
---   

[原文链接](https://wuchong.me/blog/2016/05/20/flink-internals-streams-and-operations-on-streams/)，作者wuchong，阿里花名云邪。
...
<center>{% img /images/2020/IMAG2020030101.png %}</center><br>
...
```
val stream: DataStream[MyType] = env.addSource(newFlinkKafkaConsumer08[String](...))
...
<center>{% img /images/2020/IMAG2020030102.png %}</center><br>
...
<center>{% img /images/2020/IMAG2020030103.png %}</center><br>
...
<center>{% img /images/2020/IMAG2020030104.png %}</center><br>
...
1. ConnectedStreams 连接的两个流类型可以不一致，而 union 连接的流的类型必须一致。

1. ConnectedStreams 会对两个流的数据应用不同的处理方法，并且双流之间可以共享状态。这在第一个流的输入会影响第二个流时, 会非常有用。
...
val result: DataStream[ResultType] = 
    connected.map(newCoMapFunction[MyType, AnotherType, ResultType]() {
        overridedefmap1(value: MyType): ResultType = { ... }
        overridedefmap2(value: AnotherType): ResultType = { ... }
    })
```

当并行度为2时，其执行图如下所示：

<center>{% img /images/2020/IMAG2020030105.png %}</center><br>
