---   
layout: post   
title: "大模型入门指南：Transformer与Attention机制"   
date: 2025-01-22 16:51   
comments: true   
categories: 大模型   
tags: Transformer Attention LLM 深度学习   
---   

原文链接：[https://developer.aliyun.com/article/1655110](https://developer.aliyun.com/article/1655110)

<center>{% img /images/2025/IMAG2025012201.octet-stream %}</center>
...
<center>{% img /images/2025/IMAG2025012202.webp %}</center>
...
<center>{% img /images/2025/IMAG2025012203.webp %}</center>
...
<center>{% img /images/2025/IMAG2025012204.webp %}</center>
...
# 编码示例句子
encoded_text = encoding.encode("A journey of a thousand miles begins with a single step.")print(encoded_text)
# 解码回原始文本
decoded_text = encoding.decode(encoded_text)print(decoded_text)
# 单个 token 的编码与解码
...
tokens = tokenize(prompt)  # 将提示转换为 Token 列表    
for _ in range(num_tokens):  # 根据指定的 Token 数量重复执行      
predictions = predict_next_token(tokens)  # 获取下一个 Token 的预测       
next_token = select_next_token(predictions, hyperparameters)  # 根据概率选择下一个 Token       
tokens.append(next_token)  # 将选中的 Token 添加到列表中   
return ''.join(tokens)  # 将 Token 列表合并为字符串
# 辅助函数用于选择下一个 Token
def select_next_token(predictions, hyperparameters):  
# 使用温度（temperature）、top-k 或 top-p 等策略调整 Token 选择 
# 实现细节取决于具体的超参数设置    
pass
```
...
<center>{% img /images/2025/IMAG2025012205.webp %}</center>
...
<center>{% img /images/2025/IMAG2025012206.webp %}</center>
...
1. 转换：每个输入元素（比如一个词）都会被转换成三个向量：Query (查询)、Key (键) 和 Value (值)。这些向量是通过将输入向量分别乘以三个不同的权重矩阵WQ、WK 和 WV 得到的。

2. 计算注意力分数：对于每个元素，使用它的 Query 向量与所有其他元素的 Key 向量进行点积运算，得到一个分数列表。这个分数表示当前元素与其他所有元素的相关性。

3. 归一化：将这些分数通过 softmax 函数进行归一化，得到一个概率分布，表示当前元素对其他所有元素的注意力权重。

4. 加权求和：使用这些注意力权重对所有元素的 Value 向量进行加权求和，得到最终的输出向量。
...
<center>{% img /images/2025/IMAG2025012207.webp %}</center>

- 计算注意力分数：   

<center>{% img /images/2025/IMAG2025012208.webp %}</center>

其中dk是Key向量的维度。

- 归一化：Attention Weights=softmax(Scores)
...
1. 把输入 xx 加到 F(x)F(x) 上，得到 y=F(x)+xy=F(x)+x。

1. 对 yy 进行层归一化，得到最终的输出。
...
<center>{% img /images/2025/IMAG2025012209.webp %}</center>
...
div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))        # 计算正弦和余弦值        pe[:, 0::2] = torch.sin(position * div_term)        pe[:, 1::2] = torch.cos(position * div_term)        pe = pe.unsqueeze(0).transpose(0, 1)        # 将位置编码注册为缓冲区        self.register_buffer('pe', pe)
    def forward(self, x):      
    # 将位置编码与输入相加      
