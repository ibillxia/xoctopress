...
<center>{% img /images/2019/IMAG2019061401.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019061401.webp %}</center><br>
...
<center>{% img /images/2019/IMAG2019061402.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019061403.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019061404.png %}</center><br>
...
1. 执行查询时（Restful API/JDBC/ODBC），读取 Cuboid，运算，产生查询结果。

## 2.2 体系架构

Apache Kylin 系统可以分为在线查询和离线构建两部分，技术架构如图所示，在线查询的模块主要处于上半区，而离线构建则处于下半区。

<center>{% img /images/2019/IMAG2019061403.webp %}</center><br>
...
1. 根据元数据的定义，下方构建引擎从数据源抽取数据，并构建 Cube；

1. 数据以关系表的形式输入，且必须符合星形模型（2.0 开始已经支持星型模型）；

1. MapReduce 是当前主要的构建技术（2.5 开始 Spark 是主要的构建技术）；

1. 构建后的 Cube 保存在右侧的存储引擎中，一般选用 HBase 作为存储。

### 在线查询

1. 用户可以从上方查询系统（Rest API、JDBC/ODBC）发送 SQL 进行查询分析；

1. 无论从哪个接口进入，SQL 最终都会来到 Rest 服务层，再转交给查询引擎进行处理；

1. 查询引擎解析 SQL，生成基于关系表的逻辑执行计划；

1. 然后将其转译为基于 Cube 的物理执行计划；

1. 最后查询预计算生成的 Cube 并产生结果。
...
<center>{% img /images/2019/IMAG2019061404.webp %}</center><br>
...
<center>{% img /images/2019/IMAG2019061405.webp %}</center><br>
...
<center>{% img /images/2019/IMAG2019061406.webp %}</center><br>
...
<center>{% img /images/2019/IMAG2019061407.webp %}</center><br>
...
1. 主界面顶端左侧网站 Logo 下点击加号 “＋” 以新建项目；

1. 在弹出的窗口中，输入项目名称（必选）和项目描述, 点击“确定”按钮，完成项目创建；

1. 在网站 Logo 的右侧的下拉框中选择创建的项目，点击 “Model” 菜单；

1. 点击“Data Source”选项卡，在 Tables 后面有三个按钮，第一个深蓝色按钮表示根据表名从 Hive 中导入表；第二个浅蓝色按钮点击后可以以可视化的方式从 Hive 选择需要导入的表；

1. 点击浅蓝色按钮，从 Hive 中选择需要导入的表，选择完毕后点击右下角的“Sync”按钮即可完成 Hive 表的导入。
...
<center>{% img /images/2019/IMAG2019061407.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019061408.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019061408.webp %}</center><br>
...
1. 表与表之间的连接添加有三种：“Left Join”、“Inner Join”、“Right Join”；

1. Skip snapshot for this lookup table 选项指的是是否跳过生成 snapshotTable，由于某些 Lookup 表特别大（大于 300M），如果某一个维度的基数比较大 ，可能会导致内存出现 OOM，所以在创建 snapshotTable 的时候会限制原始表的大小不能超过配置的一个上限值（kylin.snapshot.max-mb，默认值300）;

1. 跳过构建 snapshot 的 lookup 表将不能搜索，同时不支持设置为衍生维度（Derived）；

1. 大部分情况下都是使用 “Left Join”，其他两种 Join 方式不是很常用。

<center>{% img /images/2019/IMAG2019061409.png %}</center><br>

#### (2.3)、完成表关系构建

通过上述的操作即可将事实表以及维度表联系起来，构成一个数据模型。

<center>{% img /images/2019/IMAG2019061410.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019061411.png %}</center><br>
...
<center>{% img /images/2019/IMAG2019061412.png %}</center><br>
...
1. 时间分区列支持的数据类型有 time/date/datetime/integer等；

1. 过滤条件不需要写 WHERE；

1. 过滤条件不能包含日期维度。

<center>{% img /images/2019/IMAG2019061413.png %}</center><br>

### 6)、Save

最后保存即可完成 Model 的创建，你可以打开 Model 中的 Visualization 标签页查询模型的表连接情况。

<center>{% img /images/2019/IMAG2019061414.png %}</center><br>
...
1. 使用 TrieDictionary 方式对这些所有的值进行编码（一个值对应一个 Id）；

1. 再次读取原始表中每一行的值，将每一列的值使用编码之后的 Id 进行替换，得到了一个只有 Id 的新表；

1. 同时保存这个新表和 Dictionary 对象（Id 和值的映射关系）就能够保存整个维度表；

1. Kylin 将这个数据存储到元数据库中。
...
<center>{% img /images/2019/IMAG2019061413.webp %}</center><br>
...
<center>{% img /images/2019/IMAG2019061414.webp %}</center><br>

### 1、维度选择的建议：

1. 作为 Cube 的维度需要满足下面的条件：可能存在于 where 条件中或者 groupBy 中的维度；

1. 事实表（Fact Table）只选择参与查询的字段，不参与查询的一定不要勾选（即便是外键）；

1. 维度表（Lookup Table）中的主键与事实表的外键一一对应，推荐勾选事实表的外键，维度表的主键勾选后选择为衍生（Derived）维度；

1. 对于星型模型而言，维度表的字段往往可以全部为衍生字段；

1. 对于雪花模型而言，如果维度表存在子表，则维度表对于子表的外键推荐作为普通（Normal）维度。

### 2、特别注意的事项：

1. 表连接的字段并非一定要参与 Cuboid 计算；

1. 表连接的字段如果没有被勾选，且其外键表中没有任何字段作为衍生维度，则该表连接字段是不会参与 Cuboid 的；

1. 一旦被设置为 Normal 类型，则一定会参与 Cuboid 计算；

1. 如果维度表存在层级（例如省市县、日月年等），则推荐分层级的相关字段选择为普通（Normal）维度。

<center>{% img /images/2019/IMAG2019061415.webp %}</center><br>
...
<center>{% img /images/2019/IMAG2019061415.png %}</center><br>
...
1. 精确实现：基于 Bitmap（位图）算法，对于数据型为 tinyint、smallint 和 int 的数据，将把数据对应的值直接打入位图；对于数据型为 long，string 和其他的数据，将它们编码成字符串放入字典，然后再将对应的值打入位图。返回的度量结果是已经序列化的位图数据，而不仅是计算的值。这确保了不同的 segment 中，甚至跨越不同的 segment 来上卷，结果也是正确的。
...
1. 将 id 和 name 都设置为维度，并且将两者设置为联合维度（Joint Dimensions），这种方式的好处是保持维度组合数不会增加，但限制了维度的其它优化，比如 id 不能再被设置为强制维度或者层次维度；

1. 将 id 设置为维度，name 设置为特殊的 Measure，类型为 Extended Column，这种方式既能保证过滤 id 且查询 name 的需求，同时也不影响 id 维度的进一步优化。
...
1. group by country, province（等同于group by province）

1. group by country, province, city（等同于group by country, city 或者group by city）
...
1. 维度表越小越好，因为 Kylin 会将维度表加载到内存中供查询，过大的表不适合作为维度表，默认的阈值是 300MB；

1. 改变频率低，Kylin 会在每次构建中试图重用维度表的快照（Snapshot），如果维度表经常改变的话，重用就会失效，这就会导致要经常对维度表创建快照；

1. 维度表最好不要是 Hive 视图（View），因为每次都需要将视图进行物化，从而导致额外的时间开销。

### 2、事实表

1. 移除不参与 Cube 构建的字段，可以提升构建速度，降低 Cube 构建结果的大小；

1. 尽可能将事实表进行维度拆分，提取公用的维度；

1. 保证维度表与事实表的映射关系，过滤无法映射的记录。
...
1. 一般而言，Left Join 右侧表里面的字段均可以作为 Derived （衍生）维度；

1. 必需维度、层级维度、联合维度、衍生维度四种维度优化方案；
...
1. ShardBy 设置：建议选择基数较大的列作为 ShardBy 列，以使得数据可以均匀分布；

1. 数据压缩：Kylin 针对维度字典以及维度表快照采用了特殊的压缩算法，对于 HBase 中的聚合计算数据利用了 Hadoop 的 LZO 或者是 Snappy 等压缩算法，从而保证存储在 HBase 以及内存中的数据尽可能地小；

1. 对于大的事实表可以采用分区来增量构建，然后设置定期自动合并（Merge）操作；

1. 事实表中日期数据类型为尽可能设置为 date；

1. ShardBy 字段有助于全部数据分散分布在各个 Region 中，有助于防止出现数据倾斜等问题；

1. ShardBy 字段为 True 后，同一个值的数据会存储在一起，便于批量捞取数据。
...
原文链接：  
[Apache Kylin 入门 1 - 基本概念](https://juejin.cn/post/6844903693058342925)   
[Apache Kylin 入门 2 - 原理与架构](https://juejin.cn/post/6844903693599408136)   
[Apache Kylin 入门 4 - 构建 Model](https://juejin.cn/post/6844903697755947015)   
[Apache Kylin 入门 5 - 构建 Cube](https://juejin.cn/post/6844903700800995336)   
[Apache Kylin 入门 6 - 优化 Cube](https://juejin.cn/post/6844903705536364558)   
